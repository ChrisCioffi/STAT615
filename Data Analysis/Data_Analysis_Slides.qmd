...
-   Decisions made at a young age may have a profound and lasting impact on individuals for many years into the future.

-   We investigate what factors in high school impact future academic performance.

-   In particular, we ask: how is college GPA impacted by thoughts about prospective career path in high school and what drives decisions about future occupation?

    -   Originally considered using earnings as our response variable.
...
-   Predictor of Interest: Desired occupation at age 30.
...
-   Additional predictors:
    ...
    -   Demographic: Student race, Family Income
    -   School Features: High School urbanicity, High School type
...
HSLS_Data |>
  ...
  geom_boxplot() +
  labs(title = "College GPA by Desired Occupation",
       x = "College GPA",
...
HSLS_Data |>
  ...
  labs(title = "HS GPA vs. College GPA",
       x = "High School GPA",
       y = "College GPA") +
  theme_bw() +
  theme(legend.position = "none")
...
summary(SLR_GPA_Job_reg) |> broom::tidy() |>
  filter(p.value <= 0.01) |>
