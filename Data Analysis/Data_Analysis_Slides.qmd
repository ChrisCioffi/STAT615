...
HSLS_Data |>
  ...
  ggplot(aes(x = reorder(X4ENTRYMAJ23, X5GPAALL, FUN = median), y = X5GPAALL)) +
  geom_boxplot() +
  coord_flip() +
  labs(
...
HSLS_Data |>
  ...
  ggplot(aes(x = X5GPAALL, y = reorder(X1LOCALE, X5GPAALL, FUN = median))) +
  geom_boxplot() +
  coord_flip() +
...
Better, but wider at higher GPAs.

```{r}
# Use fitted values
MLR_all_WLS_fitted <- MLR_all
MLR_all_WLS_fitted$residuals <- residuals(MLR_all.reg.reduced)
MLR_all_WLS_fitted$fitted <- fitted(MLR_all.reg.reduced)
var_func <- lm(residuals^2~fitted^2, data = MLR_all_WLS_fitted)

# Obtain weights
MLR_all_WLS_fitted <- MLR_all_WLS_fitted |>
mutate(w1 = 1/abs(var_func$fitted))

# WLS model
MLR_all.reg.reduced.WLS_fitted <- lm(X5GPAALL ~ X1STU30OCC2 + X3TGPAACAD + X4ENTRYMAJ23 + X1FAMINCOME + X3TCREDAPIB + X1CONTROL, weights = w1, data = MLR_all_WLS_fitted)

# Iterate
MLR_all_WLS_fitted$residuals2 <- residuals(MLR_all.reg.reduced.WLS_fitted)
MLR_all_WLS_fitted$fitted2 <- fitted(MLR_all.reg.reduced.WLS_fitted)
var_func <- lm(residuals^2~fitted^2, data = MLR_all_WLS_fitted)

# Obtain weights
MLR_all_WLS_fitted <- MLR_all_WLS_fitted |>
mutate(w2 = 1/abs(var_func$fitted))

# WLS model
MLR_all.reg.reduced.WLS_fitted2 <- lm(X5GPAALL ~ X1STU30OCC2 + X3TGPAACAD + X4ENTRYMAJ23 + X1FAMINCOME + X3TCREDAPIB + X1CONTROL, weights = w2, data = MLR_all_WLS_fitted)

# Coefficients converge with iteration

# Plot residual variance
residuals <- rstandard(MLR_all.reg.reduced.WLS_fitted)
predicted <- predict(MLR_all.reg.reduced.WLS_fitted)
...
#summary(MLR_all.reg.reduced.WLS_fitted)
```
...
```{r}
car_plot_df <- MLR_all
car_plot_df$residuals <- rstandard(reg_career)
car_plot_df$predicted <- predict(reg_career)

# Plot residuals vs predicted values
career_plot <- ggplot(aes(x = predicted, y = residuals), data = car_plot_df) +
  geom_point(color = "blue") +
  geom_hline(yintercept = 0, color = "red") +
  labs(title = "Career Dummy Variable") +
  theme_bw()

## Credits vs GPA Non-Constant Variance
credits.reg <- lm(X5GPAALL ~ X3TCREDAPIB, data = MLR_all)

# No strong trend here so cannot be sure that there is a relationship between the variance of the error term and credits predictor variable
credit_plot_df <- MLR_all
credit_plot_df$residual_sq <- (credits.reg$resi)^2

credit_var_plot <- ggplot(aes(x = X3TCREDAPIB, y = residual_sq), data = credit_plot_df) +
  geom_point(color = "blue") +
  geom_smooth(method = "loess", color = "red", se = FALSE) +
  labs(title = "Predictor and Error Relation?",
       x = "AP/IB Credits",
       y = "Squared Residuals") +
  theme_bw()

career_plot | credit_var_plot
...
## Credits vs GPA Non-Constant Variance
credits.reg <- lm(X5GPAALL ~ X3TCREDAPIB, data = MLR_all)
plot(MLR_all$X3TCREDAPIB, credits.reg$resi)
```

```{r, fig.show='hide'}
# No strong trend here so cannot be sure that there is a relationship between the variance of the error term and credits predictor variable
plot(MLR_all$X3TCREDAPIB, (credits.reg$resi)^2)
lines(lowess((credits.reg$resi)^2~MLR_all$X3TCREDAPIB), col=2)
```
...
-   Results for WLS using fitted values
